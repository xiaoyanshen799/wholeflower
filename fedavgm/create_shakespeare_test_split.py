"""Utility to carve out a centralized Shakespeare evaluation split from client NPZ files."""
...
def collect_clients(data_dir: Path) -> List[Path]:
    """Return sorted list of client NPZ files."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split off centralized test samples from Shakespeare client NPZ files."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("fedavgm/data_partitions"),
        help="Directory containing client_*.npz files.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("fedavgm/data_partitions/test_shakespeare.npz"),
