"""Utilities for communication-efficient weight quantization."""
...
NDArray = np.ndarray
_FLT_EPS = 1e-8
_INT4_OFFSET = 8  # Shift applied when packing signed int4 values


@dataclass(frozen=True)
class QuantizationReport:
    """Lightweight container with post-quantization diagnostics."""
    ...
    def as_metrics(self) -> Dict[str, float]:
        """Return values ready to be merged into Flower metrics dict."""
        return {
...
class ErrorFeedbackQuantizer:
    """Symmetric uniform quantizer with persistent error feedback."""
    ...
    def __init__(self, num_bits: int = 8, *, error_feedback: bool = True) -> None:
        ...
        self.level = (1 << (num_bits - 1)) - 1  # e.g. 32767/127/7 for 16/8/4
    ...
    def encode(self, weights: Iterable[NDArray]) -> Tuple[List[NDArray], QuantizationReport]:
        """Quantize weights, return packed tensors and diagnostics."""

        float_weights = [self._ensure_float32(w) for w in weights]
        if self._residuals is None or len(self._residuals) != len(float_weights):
            self._residuals = [np.zeros_like(w, dtype=np.float32) for w in float_weights]
        else:
            self._ensure_residual_shapes(float_weights)
        ...
        for idx, (baseline, feedback) in enumerate(zip(float_weights, self._residuals)):
            ...
            if self.num_bits == 16:
                quantized = quantized.astype(np.int16, copy=False)
    ...
    def set_state(self, residuals: Optional[List[NDArray]]) -> None:
        """Restore residual buffers from previous round."""
    ...
    def maybe_unpack(packed: List[NDArray]) -> Tuple[List[NDArray], bool]:
        """Convert packed tensors back to float32 if needed."""
        unpacked: List[NDArray] = []
        idx = 0
        changed = False
        while idx < len(packed):
            tensor = packed[idx]
            # 16-bit / 8-bit (tensor, scale)
            if (
            ...
            # 4-bit (packed_bytes, scale, shape)
            if (
            ...
            # Fallback: not a packed tensor, preserve as-is
    ...
    def is_packed(packed: List[NDArray]) -> bool:
        """Check whether tensors follow one of the packed formats."""
...
def maybe_unpack_quantized(weights: List[NDArray]) -> Tuple[List[NDArray], bool]:
    """Public wrapper mirroring ErrorFeedbackQuantizer.maybe_unpack."""
...
def _unpack_int4(packed: NDArray, shape: Sequence[int]) -> NDArray:
    """Unpack uint8 nibbles back into signed int4 values."""
