...
# -----------------------------------------------------------------------------
# Character-level RNN for Shakespeare (sequence of codepoints -> next codepoint)
# -----------------------------------------------------------------------------

class _TiedOutput(keras.layers.Layer):
    """Output layer that ties weights to the given Embedding layer."""
    ...
    def call(self, inputs):
        # inputs: [batch, d], embeddings: [vocab, d]
        W = self.embedding_layer.embeddings
        logits = tf.linalg.matmul(inputs, tf.transpose(W))
        return logits + self.bias


def char_rnn(input_shape, num_classes, learning_rate):
    """Simple character-level model: Embedding + GRU + Dense.

    input_shape: [seq_len], dtype=int32 expected.
    num_classes: vocabulary size (max codepoint + 1, or computed by server).
    """
    # Hydra may pass ListConfig; coerce to list/int safely
    try:
        # try sequence-like
    ...
    outputs = keras.layers.Dense(int(num_classes), activation=None, name="logits")(x)

    model = keras.Model(inputs=inputs, outputs=outputs)
    optimizer = SGD(learning_rate=learning_rate)
    # Sparse labels and logits to avoid one-hot tensors and large activations
    model.compile(
