...
class CustomFedAvgM(FedAvg):
    ...
    def configure_fit(
        ...
        """Configure the next round of training with optional downlink quantization."""
    ...
    def aggregate_fit(
        ...
        # Convert results and undo client-side compression if necessary
        weights_results = []
        ...
        if server_round > 1:
            ...
            self.momentum_vector = [
                self.server_momentum * m_prev + g
                for m_prev, g in zip(self.momentum_vector, pseudo_gradient)
            ]
        else:
        ...
        fedavgm_result = [
            w - self.server_learning_rate * step
            for w, step in zip(current_weights, momentum_step)
        ]
