...
from fedavgm.dataset import cifar10, fmnist, shakespeare
...
def _load_dataset(name: str):
    ...
    if name in {"shakespeare"}:
        return shakespeare(
            num_classes=65536,
            input_shape=[80],
            data_dir=Path(__file__).resolve().parent / "data_partitions",
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset",
        default="cifar10",
        choices=["cifar10", "fmnist", "shakespeare"],
        help="Which centralized dataset to load for initial model / evaluation.",
    )
    ...
    # ------------------------------------------------------------------
    # Quantization configuration shared with clients (load from base.yaml)
    # ------------------------------------------------------------------
    base_cfg_candidates = [
    ...
    quant_cfg = {}
    for candidate in base_cfg_candidates:
        if candidate.exists():
            base_cfg = OmegaConf.load(candidate)
            quant_cfg = base_cfg.get("quantization", {})
            break

    downlink_enabled = bool(quant_cfg.get("enabled", True))
    downlink_bits = int(quant_cfg.get("num_bits", 8))
    downlink_error_feedback = bool(quant_cfg.get("error_feedback", True))
    ...
    if args.strategy == "fedavg":
        ...
        strategy = CustomFedAvgM(
            ...
            downlink_quantization_enabled=downlink_enabled,
            downlink_quantization_bits=downlink_bits,
