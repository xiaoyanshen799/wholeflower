...
MAX_DURATION = 430.0
# Boost the contribution of tail samples in the logistic fit.
# Larger values increase the weight of points with high empirical CDF.
LOGISTIC_TAIL_POWER = 3.0
TAIL_FOCUS_START = 0.9
TAIL_WEIGHT_MULTIPLIER = 10.0
LOGISTIC_PLOT_MIN_PROB = 1e-3
LOGISTIC_PLOT_MAX_PROB = 1.0 - LOGISTIC_PLOT_MIN_PROB
...
for color, (client, records) in zip(colors, client_durations.items()):
    ...
    try:
        ...
        tail_focus = np.clip((y_data - TAIL_FOCUS_START) / (1.0 - TAIL_FOCUS_START), 0.0, 1.0)
        tail_emphasis = 1.0 + TAIL_WEIGHT_MULTIPLIER * np.power(tail_focus, LOGISTIC_TAIL_POWER)
        sigma = 1.0 / (0.1 + tail_emphasis)
        ...
        # 构造平滑曲线，用来绘制拟合结果
        q_low = theta_hat + k_hat * np.log(LOGISTIC_PLOT_MIN_PROB / (1.0 - LOGISTIC_PLOT_MIN_PROB))
        q_high = theta_hat + k_hat * np.log(LOGISTIC_PLOT_MAX_PROB / (1.0 - LOGISTIC_PLOT_MAX_PROB))
        x_plot_low = min(x_data.min(), q_low)
        x_plot_high = max(x_data.max(), q_high)
        x_fit = np.linspace(x_plot_low, x_plot_high, 200)
        if plot_min is None or x_plot_low < plot_min:
            plot_min = x_plot_low
        if plot_max is None or x_plot_high > plot_max:
            plot_max = x_plot_high
...
if client_params:  # 确保至少有一个客户端成功拟合
    base_min = min(all_durations) if all_durations else None
    base_max = max(all_durations) if all_durations else None
    t_min = plot_min if plot_min is not None else base_min
    t_max = plot_max if plot_max is not None else base_max
...
plt.tight_layout()
plt.savefig("server_to_client_communication_time.png")
plt.show()
